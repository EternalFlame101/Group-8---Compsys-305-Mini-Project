...
architecture start_screen_behaviour of Start_Screen is
   ...
   signal continue_red,    continue_green,    continue_blue    : std_logic_vector(3 downto 0);
   ...
   constant CONTINUE_X     : integer := 104;
   constant CONTINUE_Y     : integer := 320;
...
begin
   ...
   -- continuously '0' for 200 ms (5,000,000 cycles at 25 MHz).  Any brief PS/2 glitch
   -- (mouse-movement packet ~3 ms, keyboard repeat gap ~90 ms) resets the counter,
   -- so spurious edges from held inputs cannot advance the screen.  Once the player
   -- genuinely releases all inputs for 200 ms the screen arms; the next fresh
   -- any_key_pressed pulse advances to mode_select.
   -- SW1+KEY2 quickstart bypasses all guards and goes directly to game_running from
   -- either title_screen or mode_select.
   ...
   begin
      if reset = '1' then
         ...
         case screen_state is
            when title_screen =>
               -- Sustained-release arming: counter increments while no key held,
               -- resets immediately if any key is detected.  Requires 200 ms of
               -- complete input silence before any key (including KEY2) can advance.
               if any_key_held = '1' then
                  title_clear_counter <= 0;
                  title_advance_armed <= '0';
               elsif title_advance_armed = '0' then
                  if title_clear_counter = 5000000 then
                     title_advance_armed <= '1';
                  else
                     title_clear_counter <= title_clear_counter + 1;
                  end if;
               end if;
               -- Fire when armed: SW1+KEY2 goes straight to game, everything else
               -- (including KEY2 alone when SW1=0) goes to mode_select.
