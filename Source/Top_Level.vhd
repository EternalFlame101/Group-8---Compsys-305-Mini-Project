-- ------------------------------------------------------------------------------
-- Top_Level
--   Top-level entity for Pusheen's Ploy. Wires the DE0-CV board I/O to every
--   subsystem: the Video_PLL and VGA pipeline (background/track/sprites/HUD via
--   the compositor), PS/2 keyboard and mouse, the Game_Master FSM, and the SD /
--   audio playback chain (SD_Init + SPI_Master + Audio_Playback out to the R-2R
--   DACs on GPIO_0).
--
--   Audio control switches: SW9 audio-reset, SW8 pause, SW7 mute, SW6-4 track
--   select. All switches pass through a synchroniser + debounce (SW_clean) before
--   use so a bounce cannot spuriously trigger an SD re-init.
--
...
architecture game_behaviour of Top_Level is
   ...
   -- Switch synchroniser + debounce (see SW_Debounce process below).
   -- SW_clean is the version every process should read instead of raw SW,
   -- so mechanical bounce / cross-switch metastability never reaches logic
   -- that latches state off a switch change (e.g. audio_sd_reset).
   constant SW_DEBOUNCE_CYCLES : integer := 250000;  -- 5 ms at 50 MHz
   signal SW_sync_0            : std_logic_vector(9 downto 0) := (others => '0');
   signal SW_sync_1            : std_logic_vector(9 downto 0) := (others => '0');
   signal SW_clean             : std_logic_vector(9 downto 0) := (others => '0');
   signal SW_debounce_counter  : integer range 0 to SW_DEBOUNCE_CYCLES := 0;
   ...
   signal track_sel_previous      : std_logic_vector(2 downto 0) := "000";
   ...
   signal endscreen_50_previous   : std_logic := '0';
   ...
   signal startscreen_fsm_previous : std_logic := '0';
   signal endscreen_fsm_previous   : std_logic := '0';
...
begin
   ...
   begin
      if rising_edge(video_clock) then
         if (not(RESET_N) = '1') then
            ...
            any_key_pressed_previous <= any_key_pressed;

            -- rising edge only
            if (any_key_pressed = '1' and any_key_pressed_previous = '0') then
   ...
   -- ===========================================================================
   -- SWITCH SYNCHRONISER + DEBOUNCE
   -- SW(9 downto 0) are raw async pins. Reading them directly (as several
   -- processes below used to) risks metastability -- flipping ANY switch can
   -- momentarily glitch the sampled value of an unrelated bit for one cycle
   -- of contact bounce -- which was intermittently kicking audio_sd_reset via
   -- Track_Reset_Stretch and re-running the whole SD init FSM (visible as
   -- LEDR3-0 flickering through initialisation states). Two-flop synchroniser
   -- into CLOCK_50, then a 5ms stable-value counter before SW_clean updates.
   -- ===========================================================================
   SW_Debounce : process(CLOCK_50)
   begin
      if rising_edge(CLOCK_50) then
         SW_sync_0 <= SW;
         SW_sync_1 <= SW_sync_0;

         if SW_sync_1 /= SW_clean then
            if SW_debounce_counter = SW_DEBOUNCE_CYCLES then
               SW_clean            <= SW_sync_1;
               SW_debounce_counter <= 0;
            else
   ...
   begin
      if rising_edge(CLOCK_50) then
         ...
         endscreen_50_previous <= endscreen_50_s2;
      end if;
   end process End_Screen_Sync;

   -- One-cycle pulse the moment the end screen appears.
   endscreen_50_rise <= endscreen_50_s2 and (not endscreen_50_previous);
   ...
   begin
      if rising_edge(CLOCK_50) then
         if track_sel_previous /= SW_clean(6 downto 4) then
            track_sel_previous   <= SW_clean(6 downto 4);
   ...
   audio_sd_reset <= (not RESET_N) or (not SW_clean(9)) or track_reset_active or endscreen_50_s2;

   -- Audio gate: silent on start screen or end screen; SW8=1 pauses; SW9=0 holds
   audio_gated_ready <= audio_ready_signal and (not startscreen_fsm) and (not endscreen_50_s2) and (not SW_clean(8)) and SW_clean(9);

   -- SW7=1 mutes both DACs (outputs x"80" = signed midpoint = silence); sector still progresses
   dac_high_out <= x"80" when SW_clean(7) = '1' else audio_dac_high;
   ...
   begin
       if rising_edge(video_clock) then
           startscreen_fsm_previous <= startscreen_fsm;
           endscreen_fsm_previous   <= endscreen_fsm;
       end if;
   end process;
   ...
   startscreen_fsm_rise <= startscreen_fsm and not startscreen_fsm_previous;
   endscreen_fsm_rise   <= endscreen_fsm   and not endscreen_fsm_previous;
   ...
   -- HEX3/HEX4 show the last raw SPI response byte (SD Error Codes)
