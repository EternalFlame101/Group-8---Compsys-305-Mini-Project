-- ------------------------------------------------------------------------------
-- Game_Master
--   Top-level game finite state machine. Sequences between the start screen,
--   training/normal gameplay, pause, and win/lose end screens, and exposes the
--   enable/FSM-phase signals the rest of the design gates on (game_enable,
--   startscreen_fsm, endscreen_fsm, pause_fsm).
--
--   Owns scoring, lives, difficulty ramp (speed_select), and win/lose decisions.
--   Instantiates Collision_Detector to turn lane/player geometry into hit and
--   gift events. god_mode and win_999 are switch-driven debug/demo overrides.
--
...
architecture game_master_behaviour of Game_Master is
   ...
   signal internal_pause_previous          : std_logic;
   ...
   begin
      if rising_edge(clock) then
         if reset = '1' then
            ...
            internal_pause_previous       <= pause_input;

            if (current_state = NORMAL or current_state = TRAINING or current_state = PAUSE) then
               if (pause_input = '1' and internal_pause_previous = '0') then
